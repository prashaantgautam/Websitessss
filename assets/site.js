document.addEventListener("DOMContentLoaded", () => {
  ...
  cvTrigger?.addEventListener("click", (e) => {
    ...
    cvTrigger?.setAttribute(
      "aria-expanded",
      String(cvMenu?.classList.contains("open"))
    );
  });

  document.addEventListener("click", (e) => {
    if (cvMenu && !cvMenu.contains(e.target)) {
      closeCV();
    }
  });

  document
    .querySelectorAll(".cv-pop a")
    .forEach((a) => a.addEventListener("click", closeCV));
  ...
  // Click/tap a photo, then use the on-image arrows,
  // keyboard arrows, or horizontal swipe to move through the gallery.

  const galleryConfigs = [
    {
      gallery: ".irrigation-gallery",
      lightbox: "#irrigation-lightbox"
    },
    {
      gallery: ".mahabharat-gallery",
      lightbox: "#mahabharat-lightbox"
    },
    {
      gallery: ".tourism-gallery",
      lightbox: "#tourism-lightbox"
    },
    {
      gallery: ".post-fire-gallery",
      lightbox: "#post-fire-lightbox"
    }
  ];
  ...
  const initGalleryViewer = ({
    ...
    const links = [
      ...document.querySelectorAll(`${gallery} .gallery-link`)
    ];
    ...
    const img = lightbox.querySelector(
      ".lightbox-figure img"
    );
    ...
    // horizontal swipe changes the photo.
  ...
  galleryConfigs
    .map(initGalleryViewer)
    .filter(Boolean);

  // Keyboard controls for image viewer
  document.addEventListener("keydown", (e) => {
    if (!activeViewer?.isOpen()) {
      return;
    }
